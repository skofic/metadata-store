...
A graph is defined by its root node: all edges whose `_path` contains that root handle belong to the graph. Filtering edges by a root handle present in `_path` isolates the edges belonging to a specific graph without creating redundant documents.
