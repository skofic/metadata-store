...
A `_predicate_required-by` edge states that the [source](_from.md) descriptor must appear in any dataset that contains the [destination](_to.md) descriptor. This expresses a co-occurrence requirement between variables in a dataset — for example, a measurement variable that is meaningless without an accompanying date or location variable.
