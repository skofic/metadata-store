...
`_tuple_type` is an array of descriptor [`_gid`](_gid.md)s. Each entry references an existing descriptor term, and the `_data` section of that term defines the type expected at the corresponding tuple position. Order is significant: position *n* in `_tuple_type` governs position *n* in the tuple value.
