...
A *tuple* is a fixed-structure ordered list where the type at each position is independently defined. Unlike an [`_array`](_array.md), which applies a uniform element type to all positions, a tuple assigns a distinct type to each index. The type at position *n* is defined by the descriptor referenced at position *n* in [`_tuple_type`](_tuple_type.md) — the shape of that descriptor's `_data` section governs the expected value.

The optional [`_elements`](_elements.md) property constrains how many elements the tuple value may contain. The length of `_tuple_type` is the absolute upper bound: neither `_min-items` nor `_max-items` may exceed it. When `_elements` is absent, the tuple must contain exactly as many elements as `_tuple_type`. When the tuple contains fewer elements than `_tuple_type`, the types for the trailing positions are not applied — the value is treated as a prefix of the full type sequence.
...
```json
{
	"_tuple": {"_tuple_type": ["ISO_3166_1_FRA", "chr_birth_date", "chr_body_weight"]}
}
```
...
{
	"_tuple": {
		"_tuple_type": ["ISO_3166_1_FRA", "chr_birth_date", "chr_body_weight"],
		"_elements": {"_min-items": 1, "_max-items": 3}
	}
}
```
