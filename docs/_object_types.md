...
`_object_types` is an optional companion to [`_scalar_type`](_scalar_type.md) when its value is [`_type_object`](_type_object.md). It constrains which object schemas the value must satisfy.
...
An object value that must conform to the `_range` schema:
