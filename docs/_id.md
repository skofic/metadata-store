...
In the dictionary, `_id` values appear wherever a document reference is needed outside the terms collection itself — as the [`_from`](_from.md) and [`_to`](_to.md) endpoints of edge documents, and as entries in the [`_path`](_path.md) set of edges. The [`_handle`](_handle.md) scalar type is used in descriptors that store `_id` values.

The field is system-managed: ArangoDB writes it at insertion and it cannot be changed thereafter. It is listed in [`_locked`](_locked.md) on the [`_term`](_term.md) schema.
