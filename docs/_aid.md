...
The property is **required** and **computed**: because `_lid` is a public identifier for the dictionary, the system initialises `_aid` to `[_lid]` if it is absent when a term is created. It is not immutable — when [alias terms](_type_term_alias.md) point to this term, their `_lid` values are appended to this set, keeping a consolidated list of all the codes by which the concept is officially known.
...
The ISO 3166-3 term for Italy carries the alpha-3 code `ITA` as its `_lid`, and the alpha-2 code `IT` as an additional official identifier in `_aid`:
