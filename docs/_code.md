...
The [local identifier](_lid.md) ([`_lid`](_lid.md)) is the only property the curator must always supply. The [namespace](_nid.md) ([`_nid`](_nid.md)) optionally scopes the term under a parent term, enabling hierarchical organisation from general to specific — for example [`ISO`](ISO.md) → [`ISO_3166`](ISO_3166.md) → [`ISO_3166_3`](ISO_3166_3.md). The [global identifier](_gid.md) ([`_gid`](_gid.md)) is computed by concatenating `_nid` and `_lid` with an underscore (`_`) separator and is unique across the entire dictionary; it is copied to the ArangoDB document key before the term is saved.

Terms with an empty-string namespace (`_nid: ""`) are core dictionary building blocks, recognisable by the leading underscore in their `_gid` — for example `_code`, [`_info`](_info.md), [`_data`](_data.md). Terms with no `_nid` property define top-level namespaces — for example `ISO`.

The [all-identifiers](_aid.md) set ([`_aid`](_aid.md)) is computed automatically from `_lid` if not supplied and grows as aliases are added. The secondary properties [`_uri`](_uri.md), [`_pid`](_pid.md), [`_name`](_name.md), [`_symbol`](_symbol.md), and [`_emoji`](_emoji.md) are optional and serve identification and presentational purposes. `_uri` records a persistent external URI that links the term to the same concept in an authority file or knowledge graph such as Wikidata or GeoNames — an identity link, not a documentation reference. [`_regexp`](_regexp.md) is also optional; when present on an enumeration root or section node it constrains the `_lid` format of the term's direct enumeration element children in the graph.
